...
class ManagerRegister extends StatefulWidget {
  String? type;
  ManagerRegister({ Key? key, required this.type }) : super(key: key);
  @override
  // // ManagerRegister createState() => _UserRegisterState();
      _ManagerRegister createState()=> _ManagerRegister();
}

class _ManagerRegister extends State<ManagerRegister>
  ...
  Future addValue() async {
    // final collucrion = await FirebaseFirestore.instance.collection('manager');
    ...
    // });
     var box = Hive.box("user");
     await FirebaseFirestore.instance.collection(widget.type!).doc(FirebaseAuth.instance.currentUser!.uid).set({
      "name":name.text,
      "institude":institude.text,
       "department":depeartment.text,
       "phone":phone.text,
  ...
  void takePhoto (ImageSource source)async{
    ...
    setState(() {
      _image= File(picarimagefile!.path);
    });
    ...
    reference.getDownloadURL().then((value){
      setState(() {
        imagepath=value;
        print(" Manager Image Link....${imagepath}");
      });
